...
(function () {
	var a, b, c, d, e, f;
	b = "0.8.5", navigator.getUserMedia = navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia, window.AudioContext = window.AudioContext || window.webkitAudioContext || window.mozAudioContext || window.msAudioContext,
		function () {
			var a, b, c, d, e, f, g, h;
		...
		}, d.prototype = Error.prototype, c = localStorage.getItem("wit_ws") || "wss://ws.wit.ai/speech_ws", a = function (a) {
			var b, c;
			...
			disconnected:
			{
				connect: function (a, b) {
					var d, f;
					return a || this.handleError("No token provided"), d = new WebSocket(c), d.onopen = function (c) {
						return function (c) {
							var f;
							return e("connection opened", c), f = {
								token: a,
								bps: 16,
			...
			ready:
			{
				...
				toggle_record: function () {
					return this.ctx || console.error("No context"), this.stream || console.error("No stream"), this.src || console.error("No source"), this.proc || console.error("No processor"), "suspended" === this.ctx.state && this.ctx.resume(), this.conn.send(JSON.stringify(["start", this.context ||
						{}])), this.rec = !0, "audiostart"
				}
			},
			audiostart:
			{
